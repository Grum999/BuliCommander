...
class BCFileManipulateNameLanguageDef(LanguageDef):
    ...
    def __init__(self):
        super(BCFileManipulateNameLanguageDef, self).__init__([
            ...
            TokenizerRule(BCFileManipulateNameLanguageDef.ITokenType.FUNCO_INT, r'\[(?:len):',
                                                                    'Function [NUMBER]',
                                                                    [('[len:\x01<value>\x01]',
                                                                            TokenizerRule.formatDescription(
                                                                                'Function [NUMBER]',
...
class BCFileManipulateName(object):
    ...
    def calculateFileName(file, pattern=None, keepInvalidCharacters=False, targetPath=None, checkOnly=False, tokenizer=None, kwCallBack=None):
        ...
        If `checkOnly`, some computation are not applied (faster but returned file name could not match real file name)
        ...
        if ast.status()==ASTStatus.INVALID:
            return (None, "".join(f"{BCFileManipulateName.__PARSER.errors()}"))
