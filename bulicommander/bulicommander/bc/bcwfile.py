...
# -----------------------------------------------------------------------------
# The bcwfile module provides widget classes that allows to manage visual
# access & rendering of file system
# --> this module is a core module for plugin
#
# Main classes from this module
#
# - BCViewFilesTv:
#       Treeview widget to render file list (list mode)
#
# - BCViewFilesLv:
#       Listview widget to render file list (icon mode)
#
# - Others:
#       Models and delegate render classes
#
# -----------------------------------------------------------------------------
...
class BCFileModel(QAbstractTableModel):
    ...
    def __updateIconsProcessed(self, processedNfo):
        ...
        if fileIndex is not None and fileIndex < self.rowCount():
            if icon is not None:
...
class BCViewFilesTv(QTreeView):
    ...
    def __headerContextMenu(self, position):
        ...
        for logicalIndex in range(1, BCFileModel.COLNUM_LAST+1):
            if logicalIndex not in (BCFileModel.COLNUM_FILE_PATH, BCFileModel.COLNUM_FULLNFO):
    ...
    def markAll(self):
        """Mark all items"""
        self.__model.setData([self.__proxyModel.mapToSource(self.__proxyModel.index(rowIndex, 0)) for rowIndex in range(self.__proxyModel.rowCount())],
                             True,
                             BCFileModel.ROLE_MARKER)
    ...
    def setIconSizeIndex(self, index=None):
        ...
        if index is None or self.__iconSize.setIndex(index):
            ...
            if self.__iconSize.index() < self.__viewNfoRowLimit:
                ...
                for logicalIndex, visible in enumerate(self.__visibleColumns):
                    header.setSectionHidden(logicalIndex, logicalIndex not in (BCFileModel.COLNUM_FULLNFO, BCFileModel.COLNUM_ICON))
    ...
    def setColumnVisible(self, column, isVisible=True):
        ...
        if column >= 0 and column < len(self.__visibleColumns) and isinstance(isVisible, bool) and column not in (BCFileModel.COLNUM_ICON, BCFileModel.COLNUM_FILE_PATH):
    ...
    def setShowPath(self, value):
        ...
        if isinstance(value, bool):
            ...
            self.__model.setColumnsVisibility([self.__visibleColumns[column] if column != BCFileModel.COLNUM_FILE_PATH else self.__showPath
                                               for column in range(BCFileModel.COLNUM_LAST)])
            self.setIconSizeIndex()
        else:
...
class BCViewFilesLv(QListView):
    ...
    def setGridNfoLayout(self, gridNfoLayout):
        ...
        if gridNfoLayout not in (BCViewFilesLv.OPTION_LAYOUT_GRIDINFO_NONE,
...
class BCViewFilesLvDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        if index.column() == BCFileModel.COLNUM_ICON:
            ...
            if self.__iconNfoRect is not None:
