...
class FontDatabase:
    ...
    def __addPaths(paths):
        ...
        for path in paths:
            if not path in FontDatabase.__paths:
    ...
    def __loadFonts():
        ...
        for dirName in FontDatabase.__paths:
            for path, subdirs, files in os.walk(dirName):
                for name in files:
                    if re.search("\.(ttf|ttc|otf|otc|pfb)", name):
                        # load font file
                        font=Font(os.path.normpath(os.path.join(path, name)))

                        if not font.type() in (Font.TYPE_NOTREADABLE, Font.TYPE_UNKNOWN):
    ...
    def __addFontName(font):
        ...
        if font.type()==Font.TYPE_OPENTYPE_TTC:
            ...
            if not fontTypoName is None and not fontTypoName in fontNameList:
                fontNameList.append(fontTypoName)

            for fontName in fontNameList:
                if not fontName in FontDatabase.__fontsByName:
...
class Font:
    ...
    def __repr__(self):
        if not self.__propFontType in (Font.TYPE_OPENTYPE_TTC, Font.TYPE_OPENTYPE_TTF, Font.TYPE_OPENTYPE_CFF, Font.TYPE_TYPE1):
    ...
    def __loadOpenType_loadTag_OS2(self):
        ...
        """
        if not Font.__TAG_OS2 in self.__tableRecord:
            return
    ...
    def __loadOpenType_loadTag_name(self):
        ...
        """
        if not Font.__TAG_NAME in self.__tableRecord:
            return
        ...
        for recordUid in fntId_nameRecords:
            ...
            if not recordUid[3] in Font.PROPERTY_NAMES:
